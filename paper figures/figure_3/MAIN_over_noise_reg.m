...
%     k            = k+1;
    ...
    %% Precond. Chebyshev
    [x,xx,~, flopc] = chebyshev_pre_iko(A,b,lam,m,x1,TOL,MAXIT, struct('k0', k0));
    err(:,k)     = err_x0(xx);
    flop(:,k)    = flopc;
...
%     [~,xx,~, flopc] = pd_mihs_v1_exact_iko(A,b,lam,[3*k0, 6*k0],x1,[TOL TOL2],[MAXIT MAXIT2], params3);
%     err(:,k)     = err_x0(xx);
%     flop(:,k)    = flopc;
...
%     [~,xx,~, flopc] = pd_mihs_v1_inexact_iko(A,b,lam,[3*k0, 6*k0],x1,[TOL TOL2],[MAXIT MAXIT2], params4);
%     err(:,k)     = err_x0(xx);
%     flop(:,k)    = flopc;
...
save data_over_noise err_mc flop_mc
% legend(names{:})