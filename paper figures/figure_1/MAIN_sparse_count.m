...
[A, b, x0, x1, dev, U, sigA, V] = generate_data(n,d,...
    ...
    'plot',                     false);
en = V'*x0;
%% Sparse example data generate
At              = sprand(round(2^4.2),round(2^2.5),0.16);
At2              = sprand(round(2^4.5),round(2^2.5),0.16);
At              = kron(At,At2);
A               = kron(At,At);
A(sum(A,2)==0,:)= [];
...
for mc = 1:MC
    ...
    [~,xx,~, flopc] = mihs_exact_iko_count(A,b,lam,m,x1,TOL,MAXIT, params1);
    err(:,k)     = err_x0(xx);
    flop(:,k)    = flopc;
    rr(:,k)      = rate_x(xx);
    names{k}     = 'M-IHS-exact';
    ...
    [~,xx,~, flopc, in_iter_mc(:,mc)] = mihs_inexact_iko_count(A,b,lam,m,x1,TOL,MAXIT, params2);
    err(:,k)     = err_x0(xx);
    flop(:,k)    = flopc;
    rr(:,k)      = rate_x(xx);
