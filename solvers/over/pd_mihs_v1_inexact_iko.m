function [x,xx, time,flopc,in_iter,params] = pd_mihs_v1_inexact_iko(A,b,lam,m,x1,tol,maxit,params)
...
if(noparam || ~isfield(params, 'k0'))
    [k0, ~, f_tr] = eff_rank_solver(WASt, WASt, lam);
end
tic;
...
while(i < 2 || (norm(x - xp)/norm(xp) >= tol(1) && i < maxit(1)))
    ...
    while(j < 2 || (norm(nu - nup)/norm(nup) >= tol2 && j < maxit2))
        ...
        %sub solve problem
        [dnu, in_iter(i),f_dx]   = AA_b_solver(WASt, g, lam, params.subtol, params.submaxit,dnu);
        ...
        nup     = nu;
        nu      = nun;
        flopc(i)= flopc(i) + f_dx + 4*m(1)*d + 7*m(1) + d; 
    end
    in_iter(i) = j;
...
flopc   = [1:k]*f_iter + cumsum(flopc) + f_rp1 + f_rp2 + f_tr;
end
