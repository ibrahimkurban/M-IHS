function [x, xx,par, B, V] = lsqr_iko_reg_test_v2(A,b,x,iter)
...
for i = 2:iter+1
    ...
    %% SVD of bidiag form
    B               = [full(spdiags([aa(1:i), bb(1:i)],[0, -1], i,i));zeros(1,i-1), bb(i)];
    [Ub,sigb,Vb]    = svd(B);
    sigb            = diag(sigb);
    Utb             = Ub(1,:)'*beta1;
    %% GCV
    gamma   = @(lam)(lam./(sigb.^2+lam));
    zheta   = @(lam)(sigb.^2./(sigb.^2+lam));
    delta0  = Utb(end)^2;
    
    fun     = @(lam)(norm(gamma(10^lam).*Utb(1:end-1))^2 + delta0)...
        /( size(B,1) - (1+size(B,2))*sum(zheta(10^lam))/n)^2;
    par_log = fminbnd(fun, -15, log10(sigb(1)), []);
